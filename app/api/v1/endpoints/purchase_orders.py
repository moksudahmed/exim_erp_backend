...
from app.schemas.purchase_order import PurchaseOrderCreate as PurchaseOrderSchema
from app.models import product as product_model
from app.models.payment import Payment
from app.db.session import get_db
from typing import List
from sqlalchemy.orm import selectinload  # Import this for loading related objects
from app.services.PurchaseService import PurchaseService
...
async def create_purchase_order(order_data: purchase_order_schema.PurchaseOrderCreate,
                                payment:PaymentCreate,
                                 db: AsyncSession = Depends(get_db)):
    service = PurchaseService(db)
    db_purchase = await service.create_purchase_order(order_data, payment)

    result = await db.execute(
        select(PurchaseOrder).options(selectinload(PurchaseOrder.items)).filter_by(id=db_purchase.id)
    )
    purchase_with_products = result.scalars().first()

    return purchase_with_products

@router.post("45564/", response_model=purchase_order_schema.PurchaseOrderResponse)
async def create_purchase_order2(
    ...
    try:
        ...
        for item_data in order_data.items:
            item = PurchaseOrderItem(
                ...
                purchase_order_id=new_order.id  # reference via ID
            )
            db.add(item)

        # Add payment associated with this purchase order
        payment = Payment(
            business_id=1,
        ...
        # Fetch and return the created purchase order with items
        result = await db.execute(
